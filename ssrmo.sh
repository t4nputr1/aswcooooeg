...
do_start(){
	...
	echo -e "${Info_font_prefix}[Informasi]${Font_suffix} $NAME (PID ${PID}) Running..." && exit 0
		...
		check_running
		if [[ $? -eq 0 ]]; then
			echo -e "${Info_font_prefix}[Informasi]${Font_suffix} $NAME Start Successfully !"
		else
			echo -e "${Error_font_prefix}[Informasi]${Font_suffix} $NAME Failed to Activate !"
		fi
	fi
}
do_stop(){
	...
	if [[ $? -eq 0 ]]; then
		...
		RETVAL=$?
		if [[ $RETVAL -eq 0 ]]; then
			echo -e "${Info_font_prefix}[Informasi]${Font_suffix} $NAME Stopped Successfully !"
		else
			echo -e "${Error_font_prefix}[Informasi]${Font_suffix}$NAME Failed to Stop !"
		fi
	else
		echo -e "${Info_font_prefix}[Informasi]${Font_suffix} $NAME Not Running !"
		RETVAL=1
	fi
}
do_status(){
	check_running
	if [[ $? -eq 0 ]]; then
		echo -e "${Info_font_prefix}[Informasi]${Font_suffix} $NAME (PID ${PID}) Running.."
	else
		echo -e "${Info_font_prefix}[Informasi]${Font_suffix} $NAME Not Running !"
		RETVAL=1
	fi
